...
async function createActive(mac, con) {
    ...
    let connection = {
        ...
        dst: con.mac,
        start_time: new Date(),
        last_update: new Date(), // we don't want to flood the DB, so updating every 1 minute
        last_seen: new Date(), // when we haven't seen the device for 5 minutes, we say that the connection has ended
